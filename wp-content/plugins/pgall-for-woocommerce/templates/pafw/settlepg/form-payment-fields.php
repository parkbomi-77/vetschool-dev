...
<div class="settlepg-payment-fields">
    ...
    <div class="pafw-card-info" style="<?php echo ! empty( $bill_key ) && ! is_account_page() ? 'display:none' : ''; ?>">
		<?php if ( 'non_ui_0' == pafw_get( $gateway->settings, 'bill_key_mode', 'non_ui_0' ) ) : ?>
        ...
        <div class="fields-wrap">
            <div class="card_no">
                <input inputmode="numeric" pattern="[0-9]*" type="text" maxlength="16" size="16" name="pafw_settlepg_card_no" placeholder="카드번호는 '-'를 빼고 숫자만 입력해주세요." value="">
            </div>
        </div>
        <div class="fields-wrap flex">
            <div class="expiry">
                <select name="pafw_settlepg_expiry_month">
					<?php
					for ( $i = 1; $i <= 12; $i ++ ) {
            ...
            <div class="expiry">
                <select name="pafw_settlepg_expiry_year">
					<?php
					for ( $i = 0; $i <= 10; $i ++ ) {
		...
		<?php if ( 'non_ui_0' == pafw_get( $gateway->settings, 'bill_key_mode', 'non_ui_0' ) ) : ?>
            <div class="fields-wrap flex">
                <div class="cert_no">
                    <div>
                        <input inputmode="numeric" pattern="[0-9]*" type="text" maxlength="10" size="10" name="pafw_settlepg_cert_no" placeholder="<?php _e( '주민번호 앞 6자리', 'pgall-for-woocommerce' ); ?>" value="">
                    </div>
                </div>
                <div class="cust-type">
                    <div>
                        <input inputmode="numeric" pattern="[0-9]*" type="password" maxlength="2" size="2" name="pafw_settlepg_card_pw" placeholder="<?php _e( '비밀번호 앞 2자리', 'pgall-for-woocommerce' ); ?>" value="">
                    </div>
                </div>
            </div>
		<?php endif; ?>
