...
function pafw_is_valid_pafw_order( $order ) {
	...
	return $payment_gateway instanceof PAFW_Payment_Gateway || ( $payment_gateway && 'bacs' == $payment_gateway->id );
}
...
function pafw_update_bill_key( $response, $order, $gateway, $user_id = null ) {
	...
	if ( $order && function_exists( 'wcs_is_subscription' ) && 'subscription' == $management_key ) {
		...
		foreach ( $subscriptions as $each_subscription ) {
			...
			$each_subscription->update_meta_data( $gateway->get_subscription_meta_key( 'auth_date' ), $response['auth_date'] );
			$each_subscription->update_meta_data( $gateway->get_subscription_meta_key( 'bill_key' ), $response['bill_key'] );
			$each_subscription->update_meta_data( $gateway->get_subscription_meta_key( 'card_code' ), $response['card_code'] );
			$each_subscription->update_meta_data( $gateway->get_subscription_meta_key( 'card_name' ), $response['card_name'] );
			$each_subscription->update_meta_data( $gateway->get_subscription_meta_key( 'card_num' ), $response['card_num'] );
