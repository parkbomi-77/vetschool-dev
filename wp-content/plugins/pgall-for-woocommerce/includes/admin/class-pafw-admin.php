...
if ( ! class_exists( 'PAFW_Admin' ) ) :
	...
	class PAFW_Admin {
		...
		static function init() {
			...
			add_action( 'add_meta_boxes_shop_order', array( 'PAFW_Meta_Box_Payment_Info', 'add_meta_boxes' ), 10, 2 );
			add_action( 'add_meta_boxes_shop_order', array( 'PAFW_Meta_Box_Cash_Receipt', 'add_meta_boxes' ), 20, 2 );
			add_action( 'add_meta_boxes_shop_subscription', array( 'PAFW_Meta_Box_Payment_Info', 'add_meta_boxes' ), 10, 2 );
			add_action( 'admin_enqueue_scripts', array( 'PAFW_Admin', 'admin_enqueue_scripts' ) );
		}

		static function admin_menu() {
			add_menu_page( __( 'PG-ALL 결제 관리', 'pgall-for-woocommerce' ), __( 'PG-ALL 결제 관리', 'pgall-for-woocommerce' ), 'manage_woocommerce', 'pafw_setting', '', PAFW()->plugin_url() . '/assets/images/mshop-icon.png', '20.876503947292' );
