...
if ( ! class_exists( 'PAFW_Gateway' ) ) {
	...
	class PAFW_Gateway {
			...
			if ( $success ) {
                ...
                <head>
                    <script>
						...
						<?php elseif( 'page' == $gateway->payment_window_mode() ) : ?>
						<?php wc_add_notice( $message, 'error' ); ?>
						<?php
						wp_safe_redirect( $redirect_url );
						die();
						?>
		...
		static function process_approval( $gateway, $order, $action = 'approval' ) {
			...
			$params = array(
				'order' => array(
					...
					'auth_token'     => wc_clean( wc_clean( $_GET['auth_token'] ) )
				)
			);
		...
		static function request_cancel( $order, $msg, $code, $gateway ) {
			$params = array(
				'order'       => array(
					'id'       => $order->get_id(),
					'txnid'    => $gateway->get_txnid( $order ),
		...
		public static function issue_bill_key( $order, $gateway ) {
			if ( 'api' == $gateway->issue_bill_key_mode() ) {
				...
				$params = array(
					'order' => array(
						'transaction_id' => wc_clean( $_GET['transaction_id'] ),
						'auth_token'     => wc_clean( $_GET['auth_token'] )
					)
				);
			}
		...
		public static function request_subscription_payment( $order, $gateway, $params = array() ) {
			...
			$card_quota           = pafw_get( $_REQUEST, 'pafw_' . $gateway->get_master_id() . '_card_quota', '00' );
