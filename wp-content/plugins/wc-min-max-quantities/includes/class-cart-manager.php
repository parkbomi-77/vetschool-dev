...
class Cart_Manager {
	...
	public static function check_cart_items() {
		...
		if ( (int) $min_order_quantity > 0 && $order_quantity < (int) $min_order_quantity ) {
			...
			Helper::add_error( sprintf( __( '장바구니에 최소 %d개 이상 담으셔야 합니다. 장바구니에 담긴 수량을 늘려주세요.', 'wc-min-max-quantities' ), (int) $min_order_quantity ) );
			remove_action( 'woocommerce_proceed_to_checkout', 'woocommerce_button_proceed_to_checkout', 20 );
		...
		if ( (int) $max_order_quantity > 0 && $order_quantity > (int) $max_order_quantity ) {
			...
			Helper::add_error( sprintf( __( '최대 허용 주문 수량은 %d개입니다. 장바구니의 수량을 줄이십시오.', 'wc-min-max-quantities' ), (int) $max_order_quantity ) );
			remove_action( 'woocommerce_proceed_to_checkout', 'woocommerce_button_proceed_to_checkout', 20 );
		...
		if ( (int) $min_order_amount > 0 && $order_total < (int) $min_order_amount ) {
			/* translators: %d: Minimum amount of items in the cart */
			Helper::add_error( sprintf( __( '장바구니에 최소 %s개 이상 담으셔야 합니다. 장바구니에 담긴 수량을 늘려주세요.', 'wc-min-max-quantities' ), wc_price( $min_order_amount ) ) );
			remove_action( 'woocommerce_proceed_to_checkout', 'woocommerce_button_proceed_to_checkout', 20 );
		...
		if ( (int) $max_order_amount > 0 && $order_total > (int) $max_order_amount ) {
			/* translators: %d: Maximum amount of items in the cart */
			Helper::add_error( sprintf( __( '최대 허용 주문 수량은 %s개입니다. 장바구니의 수량을 줄이십시오.', 'wc-min-max-quantities' ), wc_price( $max_order_amount ) ) );
			remove_action( 'woocommerce_proceed_to_checkout', 'woocommerce_button_proceed_to_checkout', 20 );
