...
class STM_LMS_Media_Library {
	...
	static function stm_lms_upload_media_library_file( $return = false ) {
	...
	public static function media_library_search_file() {
		...
		$myposts = $wpdb->get_results( $wpdb->prepare( $sql, array( $user_id, '%' . $wpdb->esc_like( $title ) . '%' ) ) );
	...
	function stm_lms_media_library_settings_page( $setups ) {
		$setups[] = array(
			'page'        => array(
	...
	function stm_lms_media_library_settings() {
